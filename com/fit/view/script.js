...
function drawGraph() {
    ...
    // Duc Anh add
    var impactSet = "[358, 363]";// giả định
    var changeSet = "[364, 357]";// giả định
    highlightSet(impactSet, 'changeset');
    highlightSet(changeSet, 'impactset');
    // End
...
function highlightItem(highlightNodes, setClass) {
    console.log(highlightNodes);
    graph.node.classed(setClass, function (d) {
...
//----------------Duc Anh add
function highlightSet(set, setClass) {
    // Bien doi set sang mang
    var items = set.replace('[', '').replace(']', '').replace(' ', '').split(',');
    var arraySet = [];
    ...
    var highlightNodes = [];
    graph.node.each(function (node) {
        var nameNode = node.name;
        /**
         * Lấy ID một Node
         */
        var beginRange = nameNode.indexOf("[");
        var endRange = nameNode.indexOf("]");
        var idNode = nameNode.substring(beginRange + 1, endRange);

        arraySet.forEach(function (impactItem) {
            if (impactItem == idNode) {
                highlightNodes.push(node);
            }
        });
    });
    highlightItem(highlightNodes, setClass);
}
//----------end