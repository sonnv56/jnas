...
public class CallGraph2 {
	...
	private void genarateGraph() {
		...
		for (int i = 0; i < nodes.size(); i++) {
			nodestring[i] = nodes.get(i).getNodeName();
			...
			if (nodes.get(i).getCallees().size() > 0) {
				for (Node callee : nodes.get(i).getCallees()) {
					for (int j = 0; j < nodes.size(); j++) {
						if (nodestring[j].equals(callee.getNodeName())) {
	...
	public int distance(Node n1, Node n2) {
		...
		List<MyEdge> path = DijkstraShortestPath.findPathBetween(unDirectedGraph, n1.getNodeName(), n2.getNodeName());
		return path.size();
	}
	...
	private ArrayList<String> getCallerSet(String method) {
		...
		for (Node caller : callerNode) {
			callerSet.add(caller.getNodeName());
		}
		return callerSet;
	}
	...
	private ArrayList<String> getCalleeSet(String method) {
		...
		for (Node callee : calleeNode) {
			calleeSet.add(callee.getNodeName());
		}
		return calleeSet;
	}
